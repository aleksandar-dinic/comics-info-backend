...
public final class ComicUseCase<APIWrapper: RepositoryAPIWrapper, CacheService: Cacheable>: UseCase where APIWrapper.Item == Comic, CacheService.Item == Comic {
    ...
    private func appendCharactersSummaries(
        ...
        return getSummaries(CharacterSummary<Comic>.self, on: eventLoop, forID: ID, dataSource: dataSource, from: table)
            .map {
                var item = item
                item.characters = $0
                return item
    ...
    private func appendSeriesSummaries(
        ...
        return getSummaries(SeriesSummary<Comic>.self, on: eventLoop, forID: ID, dataSource: dataSource, from: table)
            .map {
                var item = item
                item.series = $0
                return item
